"""Models gateway responsibilities."""
...
from redlotus.models.providers import (
    MODEL_USAGE_SINK,
    InputLimitError,
    ModelInputPolicy,
    ModelTarget,
...
class RequestPolicy(AbstractCapability):
    ...
    async def before_model_request(self, ctx, request_context):
        ...
        if "auto_compress_ratio" in target.context:
            from redlotus.models.context import compact_request_messages
            ...
            if candidate is not request_context.messages:
                if self.persist_context is None:
                    raise RuntimeError("Compressed context requires a persistence callback")
    ...
    async def after_model_request(self, ctx, *, request_context, response):
        ...
        if sink := MODEL_USAGE_SINK.get():
            await sink(self.role, response, cancelling=bool(asyncio.current_task().cancelling()))
        return response
...
async def generate_task_title(user_text: str) -> str:
    ...
    try:
        ...
        _runtime_resources.warning("LLM 标题生成失败，使用用户输入命名: %s", exc)
        return fallback
