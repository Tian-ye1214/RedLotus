...
from redlotus.cli.completer import input_completions
...
class TextualOutputSink(OutputSink):
    ...
    def update(self, action: str, *args) -> None:
        if action == "rule":
            ...
            self._run_ui(lambda: getattr(self._app, action)(*args))
...
class RedLotusTui(App[None]):
    ...
    async def _prepare_cli_session(self) -> None:
        ...
        if missing:
            from redlotus.cli.cli_commands import interactive_set_api
    ...
    def action_review(self) -> None:
        ...
        view.clear_options()
        view.add_options(
            Option(self._render_hunk(entry.name, hunk))
    ...
    def _decide_current(self, reject: bool) -> None:
        ...
        view.replace_option_prompt_at_index(
            index, self._render_hunk(entry.name, hunk, "undo" if reject else "keep")
        )
    ...
    def _all_decided(self) -> bool:
        return all(hunk.index in entry.decisions for entry, hunk in self._review_items)
    ...
    def _write_user_input(self, value: str) -> None:
        self.query_one("#output", RichLog).write(
            self._panel_renderable(value, "用户"),
            scroll_end=True,
        )

    def _write_user_reply(self, value: str) -> None:
        self.query_one("#output", RichLog).write(
            self._panel_renderable(value, "用户回复"),
            scroll_end=True,
        )
