"""Storage journal responsibilities."""
...
from redlotus.runtime.context import workspace_context
...
def _write_with_cleanup(path, operation, *, workspace=None):
    ...
    with context:
        try:
            ...
            from redlotus.storage.cleanup import retry_after_storage_cleanup
            retry_after_storage_cleanup(path, exc, operation)


class SessionJournal:
    """Locked append transactions, recovery and replay for the session JSON journal."""
    def _version(self):
        stat = self.path.stat()
    ...
    def _replace_file(path, data, *, workspace=None):
        ...
        _write_with_cleanup(path, lambda: atomic_write_json(path, data), workspace=workspace)
