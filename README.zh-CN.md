...
当前结构整改与发布验收尚未完成。充值后已恢复真实 API 验收，前一候选四入口的浏览器读取、截图和关闭通过。经批准新增显式 copy_from，复用现有审查与原子写入，保留 UTF-8 原文。源码通过 240 项辅助回归；CRLF 原题和混合字符题的真实 API 首次写入均逐字节一致，安装包和制品正在更新复测。最终长测、缓存目标和发布后验收仍未通过。测试必须包含源码、日常 pip 和实际打包入口，成功构建和隔离故障回归不能单独作为通过依据。详见[验收记录](docs/design.md#状态说明)及[开发约定](docs/development.md)。
