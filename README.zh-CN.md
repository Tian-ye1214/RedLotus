...
`1.0.1.post1` 发布流程采用标题 `1.0.1-release`，保留线上既有 `1.0.1`。本轮明确豁免 Linux、macOS、真实 QQ／微信账号及未配置模型协议的真实验收；对应功能保留，标记本轮未验证。旧版 `1.0.1` 的启动及升级测试已从范围移除。发布须完成源码、安装包和 Windows 制品验收，再核对新版公开下载。构建前检查点见[验收记录](docs/design.md#状态说明)，最终状态及制品哈希见对应的 [GitHub Release](https://github.com/Tian-ye1214/RedLotus/releases)。
