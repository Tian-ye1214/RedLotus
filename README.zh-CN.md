...
`1.0.1.post1` 候选（发布标题 `1.0.1-release`）正在 develop 完成整改和验收，尚未发布。本轮明确豁免 Linux、macOS、真实 QQ／微信账号及未配置模型协议的真实验收；对应功能保留，标记本轮未验证。旧版 `1.0.1` 的启动及升级测试已从范围移除，新版公开包的全新安装和实际发布制品下载验收仍须完成。见[当前验收记录](docs/design.md#状态说明)。

每次模型请求前，使用已报告的真实输入 Token 检查 `ceil(min(配置比例×上下文容量, 上下文容量－最大输出))`。支持持久检查点的记忆请求被服务判定超限时，沿完整证据和闭合工具调用边界分段并发压缩，摘要全部保存成功后只重试该请求一次；原始记录和 system 保留。最终候选的真实 API 验收仍在进行。
...
当前结构整改与发布验收尚未完成。充值后已恢复真实 API 验收，前一候选四入口的浏览器读取、截图和关闭通过。经批准新增显式 copy_from，复用现有审查与原子写入，保留 UTF-8 原文。源码／pip／onedir 的六题最终字节均一致，两题由模型自行读回纠正，首次错误保留。后续审查发现会话锁无限等待会阻塞取消，经批准接入现有锁超时；源码 242 项辅助回归通过，制品与最终长测正在更新。最终长测、缓存目标和发布后验收仍未通过。测试必须包含源码、日常 pip 和实际打包入口，成功构建和隔离故障回归不能单独作为通过依据。详见[验收记录](docs/design.md#状态说明)及[开发约定](docs/development.md)。
