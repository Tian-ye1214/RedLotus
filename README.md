...
Restructuring and release acceptance remain incomplete. Real API testing has resumed after funding; browser reading, screenshots, and cleanup passed on all four entries of the preceding candidate. The approved explicit copy_from option preserves UTF-8 source text through the existing review and atomic-write flow. The source candidate passes 240 auxiliary tests; both original CRLF and mixed-character real API cases produced exact bytes on the first write. Installed and frozen candidates are being rebuilt and retested. The final long-run matrix, cache target, and post-release checks have not passed. Tests must include source, daily pip, and packaged entries; builds and isolated fault checks alone are insufficient. See the [acceptance record](docs/design.md#状态说明) and [development rules](docs/development.md).
