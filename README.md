...
Restructuring and release acceptance remain incomplete. Real API testing has resumed after funding; browser reading, screenshots, and cleanup passed on all four entries of the preceding candidate. The approved character-view change passes 239 source auxiliary tests. Its original CRLF case ends with correct bytes, but the model's first write was wrong; strict copying and updated artifacts remain under validation. The final long-run matrix, cache target, and post-release checks have not passed. Tests must include source, daily pip, and packaged entries; builds and isolated fault checks alone are insufficient. See the [acceptance record](docs/design.md#状态说明) and [development rules](docs/development.md).
