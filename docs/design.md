...
| 结构／辅助检查 | 8 模块／每模块≤5 文件／最大 498 有效行；总有效行 10,372，较基线少 1。源码 84 项通过；测试自身 5 文件、最大 465 有效行；Ruff 和 whitespace 检查通过 | 逐函数必要性／正交性以及完整运行策略来源尚未全部证明 |
...
本轮证据保留在本地 `WorkDatabase/runtime/six-module-redo`：`native-final-green.log`、`clean-pip-final-regression.log`、`packages-final/inspection.json`、`final-entry-summary.json`、`current-inventory.json`、构建日志及隔离会话。制品应用代码一致；README 后续校正文案不改变这些已测代码。onefile 长路径失败位于 Python 启动前，参见 [PyInstaller 的解压文件调用](https://github.com/pyinstaller/pyinstaller/blob/v6.20.0/bootloader/src/pyi_path.c)；浅目录对照只用于定位边界，不是对失败的修复。
