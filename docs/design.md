...
| 逐函数必要性／正交性 | 原 `fab35348` 的 1,086 个 callable 全量枚举不等于证明；后续精简以差异记录衔接，当前 996 个函数＋83 个 lambda。原静态审查以各批差异证据衔接；本批删除两层终端转发，并补齐原生补全／按键／输入依据，最新仍有 28 项兼容或验收缺口，其中渠道／终端 26 项、旧记忆接口 2 项，见 `necessity-legacy-repl-delta.json`。这些证据不能代替全项目删除实验 |
