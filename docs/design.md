...
本轮已获准在 develop 完成整改与验收，全部未豁免门槛通过后合入 master。Python 包版本为 `1.0.1.post1`，标签 `v1.0.1.post1`，GitHub Release 标题 `1.0.1-release`；线上既有 1.0.1 保留。以下为构建前检查点，记录时尚未 LGTM、合并 master 或发布；该提交后续的最终状态及制品哈希以对应 GitHub Release 验收记录为准。
...
修复后的原项目 `.venv` 使用 Python 3.12.11，锁文件版本已同步；最新完整源码回归 430 项通过、无警告，发布脚本 10 项检查通过。应用为 8 模块、36 文件、10,364 有效行，低于基线 10,373；相对基线 Git 毛新增 6,645、毛删除 6,749。五个测试文件均不超过 500 有效行，公共夹具集中到既有 conftest，原断言保留。首次失败、夹具修正和 Windows 原生并发替换冲突均保留于 `WorkDatabase/runtime/post1-approved-release`；该检查点尚未完成新制品、360 回合和公开发布验收，不能据此 LGTM。下文较早候选数字均为历史证据。

真实首次配置另发现 `web_search.timelimit` 输入 `null` 会被保存为字符串并传给搜索后端。修复仅在允许 null 的字段中把显式 `null` 转为 JSON 空值，普通字符串及浏览器开关兼容值不变；新增 None／d／取消回归先有 1 失败、2 通过，修复后完整源码 286 项通过。`a43a9ab` 的四类制品及中途长测均保留为被替代候选，不能计入最终 360 回合；新候选须重新构建、核对安装制品并开始三入口长测。
...
每轮内部模型请求前，以真实输入 usage 检查 `ceil(min(配置比例×上下文容量, 上下文容量－最大输出))`；配置比例保持 0.8，当前触发值为 630,784，不新增估算器或固定轮数。具备持久检查点的主、子 Agent 和感知请求被服务明确判定超限时，沿完整证据／回合和闭合工具边界分成两段并发压缩；压缩器自身仍超限则递归二分，最小完整单元仍失败就保留原文和失败状态。各段按源顺序组成一个检查点，全部完成且保存成功后才能采用，仅重试被拒绝的模型请求一次，不重跑已完成工具。原始拒绝日志按角色、调用和请求步记录，缺失 usage 保持未知。正式记忆 20＋3 封窗不变。

`aa01681` 的真实 pip 第 68 回合出现 HTTP 400：输入 656,046 加最大输出 393,216 超过服务上限 1,048,576；上一请求尚未到自动压缩阈值，原重试条件又只允许辅助角色。修复删除该角色排除条件，复用既有一次压缩、持久化和请求重试，并记录容量拒绝以保留失败计量。源码、安装 wheel 各 286 项及三入口首次配置、四入口专项的旧证据保留；该候选不能计入最终 360 回合或用于发布。已恢复公开 master 的原样 LICENSE 并纳入 Windows 制品，避免最终合并才改变验收树。onefile 在过长的显式 TEMP 路径下仍受 Windows 解包路径限制；采用既定短临时目录的两次启动和退出清理通过，原长路径失败未删除。
