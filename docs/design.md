...
本轮重做的当前证据：47 项隔离辅助检查通过，覆盖配置三层覆盖与最小写回、基础设施独立导入、HTTP 连接池释放、磁盘清理保护、任务恢复／保存顺序、子 Agent 压缩保存、SDK 提示词拼接、记忆版本／索引完整性及分阶段提交；尚不能替代这些能力的完整真实验收。源码实际入口完成连续 3 回合 API 对话及恢复，加载列表显示 1／2／3 回合，浏览器实际打开、读取、截图和关闭通过。另以真实 Coordinator 委派复现 Worker 检查点持续写盘失败，修复后停止、清空、切项目均返回，3 个故障回合保存为取消，切项目后新对话成功；该故障在 Worker 模型请求前触发。
