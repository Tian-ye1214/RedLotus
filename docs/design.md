...
执行记录：已有整改已快进整合到 develop，本地配置指纹未变。`a1f61fea` 源码及独立安装的 `1.0.1.post1` wheel 完整辅助回归各 **233 项通过**；后续停止回执修复的源码完整回归同样 233 项通过，随后将该场景参数化为停止／清空两种代次，三项定向检查通过，源码完整 **234 项通过（83.73 秒）**，新制品仍待验证。基线曾有 209 通过、1 项在夹具原子替换时发生 Windows 权限错误，原路径的原生替换后续成功，保留原失败且不宣称已定位其环境原因。压缩定向回归先复现 12 项失败；实际 onedir 又暴露 SDK 合并相邻请求使证据边界丢失，增加原生 SDK 合并场景后先有 3 项失败，修复后 39 项定向检查通过。工作候选为 8 模块、36 个应用 Python 文件、10,365 有效行，所有文件不超过 500；相对 `ca98b811` 的应用 Python 毛新增 6,604 行、毛删除 6,725 行。结构数字不能替代业务必要性审查。
...
源码完整新人配置已用真实终端填写 102 项公共测试答案，核对类型错误拦截、角色复用、不询问容量、不落盘凭据，并完成首轮真实 API，见 `post1-new-user-source-wizard.json`；pip／onedir 完整填写仍待最终候选。`a1f61fea` 的实际 onefile 在原失败路径完成浏览器读取、截图、关闭和正常退出，冷启动 181.98 秒，当前解压目录回收。更深工作目录的会话锁路径 264 字符仍触及本机 Windows 路径限制，该失败及强制结束后的临时目录另记，不宣称任意深度均可用。

最终 360 回合、实际制品及发布后验证尚待完成。旧版 1.0.1 的相关测试已按用户最新要求停止并从当前验收范围移除，不新增兼容策略值；既有失败记录仅保留为历史证据，不再作为本轮阻塞项。新版公开 PyPI 全新安装、GitHub 下载及真实 API／恢复／浏览器冒烟验收仍须执行。过程证据位于既有隔离验收目录 `WorkDatabase/runtime/six-module-redo`，不包含私人配置副本。
