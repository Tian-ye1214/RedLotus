...
用户明确豁免本轮 Linux、macOS、真实 QQ／微信账号及未配置模型协议的真实验收；保留实现并在发布说明标明未验证。其余真实 API、360 回合、每环境三次自动压缩、缓存、资源、首用、升级、制品及架构门槛不变，历史候选不能代替最终候选。主、子 Agent 同样遵守用户工具及副作用限制；获准只向 common_conduct 补充这一通用规则，其余提示词正文及旧会话快照保留。

每轮内部模型请求前，以真实输入 usage 检查配置阈值 ×（上下文容量－最大输出）；比例保持配置的 0.8，不新增估算器或固定轮数。压缩器遇到已观测的容量拒绝时，在完整证据／回合和闭合工具调用边界二分缩小压缩前缀，剩余原文保留；候选持久化成功后才能采用，原业务请求只重试一次。正式记忆 20＋3 封窗不变。

执行记录：已有整改已快进整合到 develop，本地配置指纹未变。当前源码及独立安装的 `1.0.1.post1` wheel 完整辅助回归各 **233 项通过**；基线曾有 209 通过、1 项在夹具原子替换时发生 Windows 权限错误，原路径的原生替换后续成功，保留原失败且不宣称已定位其环境原因。压缩定向回归先复现 12 项失败；实际 onedir 又暴露 SDK 合并相邻请求使证据边界丢失，增加原生 SDK 合并场景后先有 3 项失败，修复后 39 项定向检查通过。工作候选为 8 模块、36 个应用 Python 文件、10,365 有效行，所有文件不超过 500；相对 `ca98b811` 的应用 Python 毛新增 6,599 行、毛删除 6,721 行。结构数字不能替代业务必要性审查。
...
独立取消场景先观察到运行中的 Worker，再发送 `/stop`，1.23 秒后主、子调用均取消，随后新回合正确回答、活动调用归零、进程退出。Worker 未获得 usage 的请求单列未知，不能按零消耗。初次取消驱动发现 `/status` 的方括号状态被 Rich 隐藏；改用普通括号，无新函数，真实重测见 `post1-worker-cancel-source-2.json`。首轮驱动失配、onedir 过长构建路径、混合输出编码和原超限任务的失败记录均保留。当前新 onedir 已再次恢复原任务并完成压缩，正式提交／索引仍在验证；不能提前计成功。

最终 360 回合、实际制品及发布后验证尚待完成。公开 1.0.1 已独立安装，其旧 context／model_gateway／task_title 和模型名称格式与现配置不同；升级夹具只在非敏感测试覆盖文件映射旧字段，认证沿用正常配置入口，候选应用不恢复这些废弃字段。过程证据位于既有隔离验收目录 `WorkDatabase/runtime/six-module-redo`，不包含私人配置副本。
...
用户于 2026-09-22 确认保留最大输出，自动压缩改按“（有效上下文容量－配置的最大输出 Token）×配置比例”判断，替代此前“不扣除输出预算”的约定。容量优先角色 `max_context_windows`，缺失或 null 时查询既有 OpenRouter 元数据；比例仍由配置读取。容量 1,024,000、最大输出 393,216、比例 0.9 时，在输入达到 567,706 Token 时触发，输出上限不变。界面、压缩检查和请求边界共用计算，恢复的子 Agent 使用已保存的模型输出预算。

使用最近已报告的服务端输入 Token，不把含历史思考的字符估算冒充实际用量。新增长材料在首次发送前的实际 Token 尚未知；单次新增内容仍可能超过剩余容量。用户已批准首次记忆请求明确超限后的恢复：复用压缩器、先保存候选、再重试一次，原始证据及输出上限保持。当前只识别已实测的 OpenAI 兼容接口 400 容量错误文案，其他错误继续返回；这不等于所有服务都有预检或自动恢复支持。输出上限缺失或 null 的既有用法继续交由服务决定，此时没有可扣除的明确预算，沿用总容量比例，不能声称已保证服务端输出预留。不得擅自补写输出值或降低用户配置。
...
感知 Agent 通过同一请求策略压缩，候选上下文按原会话、感知任务 ID 保存后才能采用，压缩用量单列。辅助身份本身不决定能否压缩：只有显式提供持久检查点的调用进入该路径；Compressor 自身没有该回调，不递归调用压缩器。system 指令仍保留，20＋3 的正式证据窗口不变。
...
**当前不能 LGTM。** 最新应用候选为 `f32c3217`，保留原生 onefile 解包。相对重做基线 `ca98b811`：8 个模块、36 个应用 Python 文件，每模块最多 5 文件、每文件最多 500 有效行；总计 10,348 有效行，少 25 行。逐函数必要性与完整发布门槛没有因数量达标而通过。

最新源码完整辅助回归为 **210 项通过（41.72 秒）**，隔离环境实际安装的 wheel 同样 **210 项通过（93.18 秒）**；清空源码导入路径验证安装来源，隔离安装环境依赖检查通过。测试仍为 5 文件，test_core 为 497 有效行，单文件最多 500 行；既有文件、记忆、任务、输入、取消、用量和渠道断言保留。Ruff 和 whitespace 检查通过；13 个受保护文件指纹未变，配置仅有下述已批准的三个比例值变更。此前沙箱 taskkill／TEMP 权限、夹具缺项及本轮首次驱动编码错误均单独保留，不冒充应用故障。共享日常 Python 的六项第三方依赖冲突仍按 `qq-tls-daily-pip-check.log` 保留，没有为此改动依赖。
...
| 模型输出与工具约束 | onedir 的 CRLF 指定写入和严格原样复制未通过；`ec9c2fc9` onefile 的 Worker 还使用了顶层输入禁止的额外工具。工具回执、委派内容及模型参数分别保留；后续需验证精确内容和工具使用约束，不用事后修正或新题成功替代原题通过 |
| 无工具群聊回答 | `6e1911b1` 的真实计算请求得到 DSML 工具标记正文，没有答案；没有可执行工具调用，权限未放开。后续协议对照及本批原题成功未证明原因或解决方案，原失败保留，稳定性门槛仍未关闭；不把正文解析为工具调用或修改题面取得通过 |
| 长会话与自动压缩 | 原容量冲突已有源码／pip 真实恢复通过；源码 100＋20、五窗及四次自动压缩已完成；pip 100＋20、五窗及额外 0.8 提前压缩验证完成；onedir 80 回合的压缩器容量失败未恢复。各组版本分别记录，仍欠 onedir 后续 20 主回合＋20 跨会话，不能把额外定向回合替代规定场景。每环境至少三次实际阈值压缩及最终版本验收按记录逐项关闭 |
...
| 平台、首用及升级 | Windows 定向场景及 PyPI 1.0.0→本地 1.0.1 安装升级已有证据；旧配置／数据迁移、完整新人填写及 Linux／macOS 核心真实流程未完成。三平台辅助 CI 已加入但未在远端运行；零收集为失败。候选未公开发布 |
