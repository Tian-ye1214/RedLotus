...
**当前不能 LGTM。** 应用及两个 EXE 候选为 `a902b48a`，沿用 `03acaea2` 的原生 onefile 解包。相对重做基线 `ca98b811`：8 个模块、36 个应用 Python 文件，每模块最多 5 文件、每文件最多 498 有效行；总计 10,296 有效行，少 77 行，应用 Python 毛新增 5,947、毛删除 6,181。第三方 Skills 资源另计，不能借此排除自有实现。逐函数必要性与完整发布门槛没有因数量达标而通过。

源码完整辅助回归为 **161 项通过（39.50 秒）**，日常环境实际安装的 wheel 同样 **161 项通过（37.64 秒）**；清空源码导入路径验证安装来源，另一个隔离安装环境的依赖检查通过。覆盖文件外部编辑／删除、LF 与 CRLF 字节恢复、写盘失败、记忆事务、任务取消／恢复、上下文账本、输入顺序、配置、QQ 公网地址／TLS／回退、原生类型化附件、终端中断及资源回收。本批没有增加应用或测试文件。首次受限执行为 159 通过、2 项 taskkill 权限失败，记录保留；取得所属测试进程的操作权限后原测试通过，没有修改断言或产品代码。故障注入与网络夹具使用隔离数据，不能替代真实 API、机器人账号和跨平台验收。Ruff 与 whitespace 检查通过，14 个受保护配置及提示词指纹未变。日常共享 Python 环境的 `pip check` 仍返回六项第三方依赖冲突，见 `qq-tls-daily-pip-check.log`；本次 `--no-deps` 只替换 RedLotus，没有更改冲突软件的依赖。

### 当前候选与真实入口

| 入口／制品 | 本批实际证据 | 边界 |
| --- | --- | --- |
| 源码真实调度与故障恢复 | Coordinator 委派 Worker 实际计算 229×79=18091；检查点注入一次 ENOSPC，清理日志注入一次 ENOSPC，删除过期隔离会话后保存成功。重建 AgentSystem、加载原快照后再次答出 18091；2 回合／6 次响应 | `native-cleanup-source-live.json`；当前、活动与锁定会话保留，结果和用量恢复一致，工厂与 HTTP 池归零。源夹具只证明保留结果，不证明保护候选优先被访问 |
| 日常 pip 同题故障恢复 | 实际安装 wheel 执行同一调度、故障及恢复流程；2 回合／6 次响应，旧会话删除，结果和用量保留，工厂与 HTTP 池归零 | `native-cleanup-pip-live.json`；实际导入 site-packages；活动及锁定会话分别比可删会话更旧，清理需先跳过它们 |
| onedir a902b48a | 从选择器恢复前版 20 回合会话，新增第 21 回合准确复述尺寸、排除植物及预算；既有封窗、提示词指纹和原用量不变，没有提前封存新窗口 | `a902b48a-onedir-live.json`；新增 1 回合／1 次主响应，原 20 回合不计作本批新测试；同版本候选恢复不是实际版本升级 |
| onefile a902b48a | 实际 EXE 用包内浏览器打开 Example Domain、读取标题并关闭；1 回合／5 次响应，标题单列，用量无缺失，活动 Agent 为 0，退出 0 | `a902b48a-onefile-live.json`；两个 EXE 所属进程及显式 E 盘 TMP 下解包目录回收见 `a902b48a-frozen-cleanup.json`；首次无进程读取权限的检查另存为无效结果 |
| wheel／sdist | `packages-a902b48a/inspection.json` 核对应用与资源；日常和独立环境均安装 wheel，后者依赖检查通过 | wheel SHA-256 以 `dfdba66f` 开头；同版本 1.0.1 本地候选，未公开发布 |
| 新 EXE 资源 | `frozen/a902b48a-manifest.json` 核对 33 个可达应用模块的编译代码、2,670 项资源、schema、包内 Chromium 和私人配置排除 | onedir／onefile 指纹以 `440f803b`／`2de93a43` 开头；构建使用 Python 3.12.13、PyInstaller 6.20.0；QQ／微信仍为源码／pip 入口 |

本批四入口新增 6 回合、18 次模型响应，输入 131,819／输出 2,475 Token，模型 usage 无缺失；主 Agent、Worker、标题分列在 `a902b48a-entry-summary.json`。故障注入只针对本地保存和日志，模型调用为真实服务。上述结果不能证明真实阈值自动压缩、长期资源稳定或三环境总体缓存达标。

`a902b48a` 删除 `_release_storage_lock` 包装，ExitStack 直接登记已有锁的 release；日志包装改用已有的标准库 partial。原生 Windows FileLock 已在解锁、关闭后处理锁文件删除失败，Loguru 已捕获 sink 异常；四种锁退出及实际日志 sink ENOSPC 前后对照一致。跳过释放操作的反事实检查实际导致后续加锁 Timeout，恢复原生 release 后可再次获取锁。其他原生解锁／描述符错误现在会向上传播，ExitStack 仍执行其余释放；不能把这项边界变化称为所有故障语义完全相同。证据见 `necessity-native-cleanup-delta.json`；HTTP 客户端清理的异常边界审查仍未完成。
...
| 逐函数必要性／正交性 | 当前 989 个函数＋79 个 lambda；`necessity-current-reconciliation.json` 衔接 991 个 AST 未变节点、73 个修改和 4 个新增，22 个旧定义删除或替换。`necessity-native-cleanup-delta.json` 关闭两层清理包装的依据缺口；HTTP 清理异常边界仍待审查。完全相同函数体扫描只命中两项 Protocol 声明，不能证明其他逻辑均正交或必要。旧记忆接口的外部需求、真实账号生命周期及全项目逐函数证明仍未完成 |
