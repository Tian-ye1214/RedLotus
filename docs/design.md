...
使用最近已报告的服务端输入 Token，不把含历史思考的字符估算冒充实际用量。新增长材料在首次发送前的实际 Token 尚未知；单次新增内容仍可能超过剩余容量。输出上限缺失或 null 的既有用法继续交由服务决定，此时没有可扣除的明确预算，沿用总容量比例，不能声称已保证服务端输出预留。不得擅自补写输出值或降低用户配置。
...
**当前不能 LGTM。** 最新应用候选为 `721bb827`，保留原生 onefile 解包。相对重做基线 `ca98b811`：8 个模块、36 个应用 Python 文件，每模块最多 5 文件、每文件最多 498 有效行；总计 10,310 有效行，少 63 行，应用 Python 毛新增 5,982、毛删除 6,190。逐函数必要性与完整发布门槛没有因数量达标而通过。

最新源码完整辅助回归为 **168 项通过（59.81 秒）**，日常环境实际安装的 wheel 同样 **168 项通过（54.70 秒）**；清空源码导入路径验证安装来源，隔离安装环境依赖检查通过。测试仍为 5 文件，test_core 为 496 有效行，单文件最多 500 行；既有文件、记忆、任务、输入、取消、用量和渠道断言保留。Ruff、whitespace 及 14 个受保护文件指纹检查通过。本轮首次普通沙箱运行有两项 taskkill 权限失败，随后在具备测试子进程清理权限的环境全部通过；两次结果均保留。共享日常 Python 的六项第三方依赖冲突仍按 `qq-tls-daily-pip-check.log` 保留，没有为此改动依赖。
...
| 最终 EXE 构建 | onedir／onefile 均核对 33 个可达模块、2,670 项资源、Chromium 和私人配置排除；指纹分别以 `aaf9d53a`／`a395ffac` 开头，见 `frozen/721bb827-manifest.json` |
| 最终 onefile 实际入口 | 三轮真实 API：Worker 算得 437；浏览器读取隔离页面、截图并关闭；退出后恢复并答对页面回执。保留 system 和既有账本，两次进程正常退出，未遗留该工作区进程或 `_MEI`。`input-budget-onefile-verified-progress.json` 与 `input-budget-onefile-independent-check.json` 分别记录运行与独立核对 |
...
pip 和 onedir 的 100＋20 仍在执行。新窗口测试使用每回合 800 条合成订单，原 1,500 条失败场景另行完成真实恢复复测，原失败没有被替换。真实模型会自行选择额外工具：两个环境在首个自动记忆窗口后频繁调用更新记忆，耗时显著增加，不能把这部分等待或付费用量从统计删除。最终门槛按实际窗口、摘要事实、调用账本、缓存与资源判定；未完成部分仍是待办。
...
| 逐函数必要性／正交性 | 当前 991 个函数＋79 个 lambda；`necessity-current-reconciliation.json` 对齐的是 `ec9c2fc9` 的 1,069 项：979 个 AST 未变、83 个修改、7 个按名称新增，24 个旧定义删除或替换。本次新增一个共享阈值函数，另附 `necessity-input-budget-delta.json`。旧记忆兼容接口保留依据已核对；真实账号生命周期及全项目逐函数证明仍未完成。相同函数体扫描不能证明所有逻辑均必要或正交 |
...
| 长会话与自动压缩 | 原容量冲突已有源码／pip 真实恢复通过；源码 100＋20、五窗及四次自动压缩已完成，pip／onedir 继续执行。每环境 100 主会话＋20 跨会话、合计 360 回合、五个自动记忆窗口及每环境三次实际阈值压缩，按最终记录逐项关闭 |
