...
**当前不能 LGTM。** 应用候选为 `519ec199`，相对重做基线 `ca98b811`：8 个模块、36 个应用 Python 文件，每模块最多 5 文件、每文件最多 498 有效行；总计 10,324 有效行，少 49 行，应用 Python 毛新增 5,937、毛删除 6,116。第三方 Skills 资源另计，不能借此排除自有实现。逐函数必要性与完整发布门槛没有因数量达标而通过。

源码完整辅助回归为 **153 项通过（35.09 秒）**，已安装 wheel 的独立环境同样 **153 项通过（43.18 秒）**，该环境依赖检查通过。覆盖文件外部编辑／删除、LF 与 CRLF 字节恢复、写盘失败、记忆事务、任务取消／恢复、上下文账本、输入顺序、配置、QQ 公网地址／TLS／回退及资源回收。故障注入与网络夹具使用隔离数据，不能替代真实 API、机器人账号和跨平台验收。Ruff 与 whitespace 检查通过，14 个受保护配置及提示词指纹未变。日常共享 Python 环境的 `pip check` 仍返回六项第三方依赖冲突，见 `qq-tls-daily-pip-check.log`；本次 `--no-deps` 只替换 RedLotus，没有更改冲突软件的依赖。
...
| wheel／sdist | `packages-qq-tls/inspection.json`：全部 36 个应用文件及 schema 与 `519ec199` 一致，无私人配置；日常 pip 和独立环境均安装 | 同版本 1.0.1 候选替换，不是公开新包发布或真实版本升级 |
...
| 日常 pip 实际入口 | 恢复源码的 3 回合会话，显式 `max_results=1` 实际返回 1 条；读回撤销后的 before，原 LF 字节未变；新增 1 回合／2 次响应，累计 4／9；`/usage`、`/status` 及退出正常 | 与源码共用隔离测试会话，未读正式数据 |
| onedir 实际 EXE | 包内浏览器打开 Example Domain 并关闭；两次真实回合新增 16 次响应，累计 10 回合／59 次响应，usage 无缺失、活动 Agent 0、退出 0 | **CRLF 指定写入及模型原样复制未通过**：首轮借助额外 shell 修正，第二轮读回返回含 CRLF，模型却生成 LF 参数；不能以最终文本相同宣称通过 |
| onefile 实际 EXE | 浅目录恢复 7 回合，write_file 写出精确 `east\nwest\n` 的 10 字节 LF 文件，读回正确，旧 `42` 文件未变；浏览器打开并关闭；新增 1 回合／4 次响应，累计 8／48，退出 0、解压目录清空 | 深目录失败仍未解决，浅目录对照不能替代 |
| PyInstaller 资源核验 | `frozen/146af307-manifest.json`：两个 EXE 的 33 个 CLI/TUI 可达应用模块与提交的原生代码对象一致，schema 和 Chromium 存在，私人文件未入包 | QQ／微信既有入口仍为源码／pip 模块；构建保留已解释的可选 pycparser 表及 pandas Styler/jinja2 警告 |

当前 wheel／sdist 的 SHA-256 分别以 `04231557`／`5e573b14` 开头，完整指纹见 `packages-qq-tls/inspection.json`。本批只改 QQ 附件模块，两个既有 EXE 中的 33 个 CLI/TUI 可达应用模块及 schema 与当前提交不变；`frozen/519ec199-compatibility.json` 记录该核对和 EXE 指纹未变，没有重新构建。`146af307-entry-summary.json` 是前批三个实际入口的测试记录，不能充作本批新增回合。onedir／onefile EXE 的 SHA-256 仍以 `71005b8d`／`f88d8130` 开头。
...
| 逐函数必要性／正交性 | 原 `fab35348` 的 1,086 个 callable 全量枚举不等于证明；后续精简以差异记录衔接，当前 998 个函数＋83 个 lambda。原 117 项中补齐 75 项具体源码／SDK 及行为依据、删除 1 项重复入口、以真实原生 UI 补齐 8 项；本批再以真实 DNS／下载补齐 1 项，仍有 32 项兼容或验收缺口。新增的单个 TLS 适配函数有具体失败和复用依据，见 `necessity-qq-tls-delta.json`；不能代替全项目删除实验 |
| 运行策略来源 | 已关闭原列出的轨迹、刷新、diff、重定向、图像、流式预览、会话展示、补全和搜索策略；通知／明细上限、名称长度及 CSV 探测也来自配置。渠道目录 20 字符短名及固定展示布局继续做兼容评估，身份／协议格式不随意参数化 |
