...
> 基线：`develop@d6bcc414`。本文同时记录代码现状与已确认目标；两者不一致时明确标出，不通过文档改写功能口径。
...
按键检测与对话隔离，启动可跳过，TUI 可重测；保护草稿，不创建会话、不调用模型、不持续记录按键。仅 Enter 与 Ctrl+Enter 分别承担两种提交行为；不使用 LF／Ctrl+J 兼容加急，不恢复文字加急命令。Windows Terminal 在用户批准的独立编码设置后已物理验证；PyCharm Classic 的事件不可区分问题仍存在，新环境需实际检测。程序不会自动修改终端设置。
...
| 结构与功能等价 | 五板块已形成，文件数／有效行仍超标；完整功能等价门槛未全部关闭 |
