...
| `tools` | 5／469 | 身份工具集合、注册和执行检查、文件引用、文档解析、浏览器及 Skills | 已实现 |
...
| `ui` | 5／498 | CLI／TUI 控制、命令、展示及 Textual 原生控件 | 五个文件；展示通过入口注入；控件共享同一输入及会话控制器 |
...
| 结构／辅助检查 | 8 模块／每模块≤5 文件／最大 498 有效行；总有效行 10,328，较基线少 45。源码 100 项通过；Ruff 和 whitespace 检查通过 | 逐函数必要性／正交性以及完整运行策略来源尚未全部证明 |
...
2026-09-21 后续真实验收与精简：删除五个未接入运行流程的旧记忆方法，保留正式记录、投影、版本化索引及实际旧格式读取。辅助测试的一处文件时间夹具在 exFAT 上失败，改为两天前而非 1970 年；这是测试修正。最新 92 项源码回归通过（31.91 秒），测试有效行最大 488。

感知复用 Worker 模型时，原来依据压缩参数推断用量类别，实测把感知计入普通子 Agent。现由调用入口声明用途，辅助调用也不继承会话自动压缩行为。真实新会话 `8e23a68b2a7246ad84b1c9b1c00f2502` 保存全局偏好并投映画像，6 次响应、无缺失 usage；感知为辅助类别（17,633 输入／13,042 输出），标题为辅助类别（312／23），主 Agent 为 23,488／576。历史失败验收的错误分类保持原证据，不用后续正确结果覆盖。

负向验收曾发现：用户要求“仅当前项目”即时记忆时，Coordinator 调用全局 L2 的 `remember`，感知保存后才发现范围错误并删除。旧记录仍保留 deleted v2 失败证据。现保持“手动全局 L2、自动项目 L1”边界，给 `remember` 增加必填作用域，项目请求在模型生产和正式写入前返回拒绝。原题真实重测没有生产或误保存；随后独立的明确全局请求正常生成 `ca62465e3898ccc96592de38f23e6f7b`。隔离会话共 2 回合、9 次响应、1 个全局生产任务，证据为 `memory-scope-live.json`。这不表示新增了手动项目记忆能力，未修改受保护提示词。

渠道保存重试及五个旧方法删除后的中间制品位于 `packages-delivery` 和 `frozen/*-delivery`：日常 pip 真实两消息写盘恢复通过，干净安装 85 项回归通过，onedir 与浅目录 onefile 均恢复旧会话、读回未变的文件、重新打开并关闭浏览器。该批制品早于感知用量分类修复，不作为该修复的验收证据。

`packages-usage` 对应 `6d5f6e83` 的分类修复：wheel／sdist 内 36 个应用文件与该批源码逐字节一致，字段契约存在，私人配置与会话未入包；日常 pip 和既有干净环境覆盖安装成功，干净安装 92 项回归通过（33.38 秒），依赖检查通过。日常 pip 恢复 `8e23a68b2a7246ad84b1c9b1c00f2502`，检索到已提交偏好，再由 Worker 实际计算 23+19=42；2 回合／11 次响应，主、普通子 Agent、感知与标题分类及累计值保持正确，正常退出。
...
`815004f1` 的制品位于 `packages-policy`、`frozen/*-policy`，哈希见对应 inspection／manifest；源码、字段契约和资源检查通过，私人配置未入包。日常 pip 与干净 pip 均安装，干净安装 99 项回归通过（43.41 秒）、依赖无冲突。日常入口恢复第 3 回合后，Worker 实际计算 17+25=42，项目限定记忆在写入前被拒绝。onedir 与浅目录 onefile 都在恢复后实际打开包内浏览器、按配置 200 毫秒超时并关闭，活动 Agent 归零、正常退出，onefile 解压目录清理完成。这批制品不包含之后的 QQ 时限复用及无用参数／包装删除。

最终终端精简后的 `delivery-final-green.log` 为 100 项通过（36.03 秒）；11 组金额与修改前输出逐项一致。真实感知更新原记录 `b8e2ec4564b2c798a187a503659a6dba` 至 active v2，正式库仍只有该记录；`/usage` 显示 23 次累计响应、无缺失 usage，主、普通子 Agent、辅助角色分列。已删除无用参数、属性转发及三处不必要的展示包装。真实源码再次恢复 5 回合会话，`/agent`、`/effort`、`/stop` 展示正常，真实模型回复后正常退出、活动 Agent 为 0；加载回合数继续累计。逐函数证据汇总仍须区分静态推导与实际删除验证。

**QQ 代理下载尚未通过：**真实下载器在配置极短时限下返回文件名、输入 ID 和失败原因，未创建 Agent／模型请求。恢复正常时限后，同一 python.org 文件按域名访问成功，固定到已校验 IP 的 HTTPS 下载却失败。诊断跟踪确认本次验收代理隧道用 IP 校验证书，未采用请求中的 SNI；[HTTPX 的 SNI 扩展](https://www.python-httpx.org/advanced/extensions/#sni_hostname)描述了直接 IP 访问时的主机名用途，但当前安装的代理路径没有相同行为。IPv6 连接还出现 TLS EOF。未关闭证书校验、移除公网地址检查或绕过代理；临时原生 TLS 继承原型也未解决 IPv6 失败，没有并入应用。`qq-policy-source-live-first.*` 保留验收脚本误引用属性的首轮失败，后续日志及 `qq-tls-*.json` 保留真实网络失败。这个成功下载／重试场景和真实 QQ 账号收发都保持未完成，不能宣布整体 LGTM。

`8a3fdac3` 的 wheel／sdist 与该提交的全部 36 个应用文件一致；同提交 onedir 的 33 个 CLI／TUI 可达应用模块经代码对象核对一致，浏览器及字段契约存在，未包含私人配置。QQ／微信的既有入口是源码／pip 的 `python -m redlotus.api.QQ`／`WeChat`，不由这个 CLI EXE 启动。这一批没有执行安装或真实运行，也不包含随后终端包装精简，不能沿用上一批运行结果作为它的通过证据。

`fab35348` 是终端精简后的应用提交：`packages-fab35348/inspection.json` 核验 wheel／sdist 的全部应用文件和字段契约，`frozen/fab35348-manifest.json` 核验两种 EXE 的 33 个可达应用模块、包内 Chromium 和凭据排除。日常 pip 和既有干净环境均安装该 wheel；干净安装依赖检查通过，100 项安装代码回归通过（43.40 秒）。源码完整回归为 100 项（36.03 秒）。

同一提交的真实入口结果保存在 `fab35348-entry-summary.json`：日常 pip 恢复后由 Worker 用日常 Python 只读核对旧文件仍为两个字节 `42`，新增 1 回合／5 次模型响应；onedir 实际打开 Example Domain、读取标题并关闭，新增 1 回合／4 次响应；onefile 浅目录控制组读回旧文件并打开／关闭浏览器，新增 1 回合／3 次响应。三个入口最终活动 Agent 均为 0、退出码 0、无缺失 usage；onefile 解压目录已清空。账本累计回合分别为 7／6／5，包含此前版本，不能全部计作当前制品新增验收。onedir 首次误指向不存在的测试配置，已在向导首问取消且未写盘，随后使用原有公开测试策略完成；不是应用故障，也不隐藏这次测试操作失误。

上述制品仍为本地 1.0.1 候选，未发布、推送或合并。当前源码数量约束已通过；逐函数必要性、剩余运行策略来源、QQ 代理下载、onefile 深目录和原全量发布验收继续按各自证据保持未完成。

以 `fab35348` 为基线的静态审查覆盖 1,086 个函数／lambda 位置。随后合并模型选项读取及原子写入，删除单次缓存路径包装，净减少 5 个函数、11 有效行；关闭七项重复实现缺口及文件锁策略缺口，117 项证明或验收缺口仍未关闭，详见[开发约定](development.md#代码组织与精简)。文件锁复用 `storage.file_lock_timeout_seconds`，首次配置使用待提交值。当前源码完整辅助回归为 107 项（37.71 秒），真实终端完成单字段配置、恢复和引用输入模型调用；打包制品的已通过基线仍是前述 `fab35348`，不能沿用旧制品证据宣布新源码全部可合并。
