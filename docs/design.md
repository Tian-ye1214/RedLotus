...
| 板块 | 职责与边界 | 状态 |
| --- | --- | --- |
| `core` | Agent 编排、会话与事务、任务、压缩和用量统计 | 已实现；终端展示及任务状态归属仍在收敛 |
| `tools` | 身份工具集合、注册和执行检查、文件引用、文档解析、浏览器及 Skills | 已实现 |
| `api` | CLI 启动与配置向导、QQ／微信消息接入、本人身份判断、附件和展示 | 启动入口已迁入；终端展示迁移与真实渠道验收未完成 |
| `prompts` | 提示词资源、组装、系统信息及会话快照 | 已实现；正文变更仍受单独审批约束 |
| `memory` | 记忆生产、证据与消费、LanceDB、embedding／rerank 和索引恢复 | 已实现；完整质量门槛未完成 |
| `runtime` | 配置、工作区路径、日志、HTTP 客户端与模型连接、锁及原子 I/O | 已收敛为四个文件；独立导入不加载应用层，会话清理仍由 core 决策 |

CLI/TUI 和渠道适配器进入统一的 AgentSystem 会话接口。工具通过工厂获得调用身份和工作区；记忆服务使用会话证据和统一模型入口。代码规模要求及现有超标项见[开发约定](development.md#代码组织与精简)。

以上六个模块是当前实现。用户已允许在职责确有必要时新增一至两个模块，总数最多八个；每个新增模块必须说明独立职责和依赖收益，不能仅拆文件凑数。重做保留已有功能，应用有效行必须少于基线 10,373 行，代码删除行数大于新增行数；逐函数以职责、调用关系和删除后具体行为损坏的证据验收。不得挤行、将业务实现藏入资源／测试、硬删功能或人为制造崩溃来达标。功能改造和测试审批均执行同一约束。
...
本轮重做的当前证据：57 项隔离辅助检查通过，覆盖配置三层覆盖与最小写回、基础设施独立导入、HTTP 连接池释放、磁盘清理保护、任务恢复／保存顺序、子 Agent 压缩保存、SDK 提示词拼接、记忆版本／索引完整性及分阶段提交，以及渠道输入顺序、附件失败和非本人会话计数；尚不能替代这些能力的完整真实验收。源码实际入口完成连续 3 回合 API 对话及恢复，加载列表显示 1／2／3 回合，浏览器实际打开、读取、截图和关闭通过。另以真实 Coordinator 委派复现 Worker 检查点持续写盘失败，修复后停止、清空、切项目均返回，3 个故障回合保存为取消，切项目后新对话成功；该故障在 Worker 模型请求前触发。

渠道共用 core 输入控制器，在下载前分配输入身份、顺序及代次。微信适配器至真实模型回放验证附件先于后续文本、单个附件失败不调用模型、停止保留独立队列、清空丢弃旧输入；完成回合数分别为 2、3 和新会话的 1。入站 SDK 消息和附件是隔离夹具，回复只写本地，未登录或发送机器人消息。首轮有一次未分类的真实流式失败，日志与不完整用量保留，第二轮同场景通过。真实回放还发现非本人渠道不累计回合，现已修复；个人记忆及执行工具的本人权限继续独立校验。

最新真实 Manager／Worker 场景保存 A 待输入、B 待依赖、C 完成；重启后的第一次恢复因模型尝试不存在的能力而失败，准确保留为未验证且未自动重试。修复提示词重复并明确补充后，再次重启恢复 A，随后执行 B，C 的输入边界、完成结果及文件字节保持不变。调度及恢复链路已有真实证据；C 的首次产物有行尾空格，B 返回 CSV 正文而非格式名称，仍保留为模型交付偏差，不能称该题全项通过。隔离记忆库另完成真实 embedding 建索引及同义检索。OpenRouter 实际元数据查询已验证容量缺失与 null 的回退；完整结构、首次配置、用量补齐、渠道输入、pip／EXE 及发布门槛仍未完成。

基础设施迁移后，源码入口另完成 3 回合真实 DeepSeek 验收：主 Agent 回复、实际 Worker 浏览器打开／读取标题／关闭、正常退出并重启恢复。加载列表显示 2 → 3 回合，Coordinator 静态提示词指纹保持一致。账本保留主 Agent 4 次响应（31,756 输入／620 输出 Token）和 Worker 5 次响应（38,748 输入／733 输出 Token）；标题用量尚未补齐，不计入这些数值。当前共 32 个应用 Python 文件、10,400 有效行；core 9 文件、tools 7 文件且部分文件超过 500，有效行净减少与模块内部文件数的最终门槛仍未关闭。
