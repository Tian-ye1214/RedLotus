...
**当前不能 LGTM。** 应用及两个 EXE 候选为 `69b23423`，沿用 `03acaea2` 的原生 onefile 解包。相对重做基线 `ca98b811`：8 个模块、36 个应用 Python 文件，每模块最多 5 文件、每文件最多 498 有效行；总计 10,293 有效行，少 80 行，应用 Python 毛新增 5,949、毛删除 6,187。第三方 Skills 资源另计，不能借此排除自有实现。逐函数必要性与完整发布门槛没有因数量达标而通过。

源码完整辅助回归为 **163 项通过（41.84 秒）**，日常环境实际安装的 wheel 同样 **163 项通过（39.54 秒）**；清空源码导入路径验证安装来源，独立安装环境依赖检查通过。本批把四份同形依赖检查合并为参数化测试，原四种隔离导入条件全部保留，共享重复导入；现有 HTTP 池回归增加外部取消、原生关闭中断和意外错误，测试文件仍为 5 个，单文件最多 500 有效行，没有新增文件。原有文件、记忆、任务、输入、用量及渠道回归继续通过。Ruff 与 whitespace 检查通过，14 个受保护配置和提示词指纹未变。故障注入与网络夹具不能替代真实 API 或跨平台验收；共享日常 Python 的六项第三方依赖冲突仍按此前 `qq-tls-daily-pip-check.log` 保留，没有为此改动依赖。

### 当前候选与真实入口

| 入口／制品 | 本批实际证据 | 边界 |
| --- | --- | --- |
| 源码真实调度与恢复 | Coordinator 委派 Worker 实际计算 241×83=20003，恢复同一会话后再次答出 20003；2 回合／6 次响应，结果和用量保留，HTTP 池与工厂归零 | `http-cleanup-source-verified.json` 从原会话核对；两次探针断言失败原样保留，模型请求已成功。未重复原计算来覆盖失败记录 |
| 日常 pip 同题验收 | 实际安装 wheel 完成同题调度与恢复，2 回合／6 次响应；Worker finally 和入口清理的取消回执及资源释放核对通过 | `http-cleanup-pip-live.json`；模型 socket 在探针执行时已经关闭，不能当作活跃模型连接取消的证据 |
| 原生 HTTP 取消对照 | 两个实际 keepalive 客户端：初版取消后第二个未关闭；同一客户端的两个实际连接：只用 AsyncExitStack 仍遗留一个 socket。复用既有 I/O 收尾后均关闭，取消仍传播 | `http-cleanup-before-controls.json`、`http-cleanup-shared-client-red.json` 与对应通过记录；这是实际本地 HTTP 连接，不是付费模型调用 |
| onedir 69b23423 | 选择器显示并恢复 21 回合，新增第 22 回合算出 96 平方米及每平方米 9.375 credits；原封窗、提示词和已保存用量不变 | `69b23423-onedir-live.json`；新增 1 回合／1 次响应，旧 21 回合不重复计数。未触发自动压缩，不是实际版本升级 |
| onefile 69b23423 | 中文、含空格 CSV 与第二份文本引用解析成功，快照字节与原件相同；模型生成脚本，纠正一次错误路径后得出 91、87、总计 178。1 回合／10 次响应，标题单列，usage 无缺失 | `69b23423-onefile-live.json` 保留首次命令退出 1：模型使用 64 字符散列推导目录，实际目录为 32 字符。最终正确不能代表首试无错误；使用 PATH 中外部 Python |
| wheel／sdist | `packages-69b23423/inspection.json` 核对 36 个应用源文件、资源及私人配置排除；日常和独立环境均安装 wheel，后者依赖检查通过 | wheel SHA-256 以 `02b50828` 开头；同版本 1.0.1 本地候选，未公开发布 |
| 新 EXE 资源与退出 | `frozen/69b23423-manifest.json` 核对 33 个可达应用模块的编译代码、2,670 项资源、schema、Chromium 及私人配置排除；两个 EXE 均退出 0，所属进程和 onefile 解包目录回收 | onedir／onefile 指纹以 `73c970ab`／`6b4861fc` 开头；Python 3.12.13、PyInstaller 6.20.0，onefile 使用当前进程显式 E 盘 TMP。不是任意路径或跨平台通过 |

本批新增 6 回合、23 次模型响应，输入 195,095／输出 5,703 Token，usage 无缺失；主 Agent、Worker、标题分列在 `69b23423-entry-summary.json`。源码首轮探针要求已经关闭的 socket 仍打开，续验又在清理回调触发前断言已有观测；两次原失败保留，均未覆盖活跃模型连接取消。真实模型完成状态另从原会话核对，没有重跑已完成的计算任务。正常模型调用与本地原生 socket 取消证据相互补充，仍不能关闭三环境长测、缓存和全部取消边界。

`69b23423` 删除唯一由 `close_all_clients` 调用的 `close_client`，使用 AsyncExitStack 处理各客户端的释放，移除当前循环的池记录；不关闭其他线程／循环的池。既有 `finish_file_io` 改名为 `finish_io`，可执行函数体不变，文件、引用及记忆保存仍使用同一实现，HTTP 清理新增复用这一收尾边界。意外错误在执行其余释放后向上传播，不再被宽泛吞掉。`necessity-http-cleanup-delta.json` 记录故障、消费者、删除损坏及证据限制；没有新增取消框架、模型策略或模块。
...
| 逐函数必要性／正交性 | 当前 988 个函数＋79 个 lambda；`necessity-current-reconciliation.json` 衔接 982 个 AST 未变、80 个修改、5 个按名称新增，24 个旧定义删除或替换。第五项是原 I/O 函数改名，函数体未增加。`necessity-http-cleanup-delta.json` 关闭 HTTP 清理的宽泛异常吞并缺口；旧记忆接口的外部需求、真实账号生命周期及全项目逐函数证明仍未完成。相同函数体扫描不能证明所有逻辑均必要或正交 |
