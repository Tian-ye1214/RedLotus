...
**当前不能 LGTM。** 应用候选为 `45937e81`，相对重做基线 `ca98b811`：8 个模块、36 个应用 Python 文件，每模块最多 5 文件、每文件最多 498 有效行；总计 10,306 有效行，少 67 行，应用 Python 毛新增 5,956、毛删除 6,177。第三方 Skills 资源另计，不能借此排除自有实现。逐函数必要性与完整发布门槛没有因数量达标而通过。

源码完整辅助回归为 **157 项通过（35.49 秒）**，已安装 wheel 的独立环境同样 **157 项通过（39.87 秒）**，该环境依赖检查通过。覆盖文件外部编辑／删除、LF 与 CRLF 字节恢复、写盘失败、记忆事务、任务取消／恢复、上下文账本、输入顺序、配置、QQ 公网地址／TLS／回退、原生类型化附件、终端中断及资源回收。故障注入与网络夹具使用隔离数据，不能替代真实 API、机器人账号和跨平台验收。Ruff 与 whitespace 检查通过，14 个受保护配置及提示词指纹未变。日常共享 Python 环境的 `pip check` 仍返回六项第三方依赖冲突，见 `qq-tls-daily-pip-check.log`；本次 `--no-deps` 只替换 RedLotus，没有更改冲突软件的依赖。

### 当前候选与真实入口

| 入口／制品 | `45937e81` 实际证据 | 边界 |
| --- | --- | --- |
| 源码 QQ 原生 SDK 至模型 | ncatbot 4.4.1.post1 的原生注册、事件分派、`run_frontend`／`start` 均实际执行；私聊 read_file、原生 File 文件 ID 经真实 HTTP 下载 537 字节并回答首行、定向群聊计算共 3 回合／4 次响应，usage 无缺失，退出后 HTTP 池为 0 | `qq-sdk-native-reply-source-live.json`；NapCat 启动、文件 URL 返回和收发传输是本地夹具，没有登录账号 |
| 日常 pip 的同一 SDK 场景 | 安装新 wheel，确认模块来自日常环境 site-packages 且源码指纹相符；原题文件工具、附件回答和群聊计算全部通过，3 回合／4 次响应，usage 无缺失，退出后 HTTP 池为 0 | `qq-sdk-native-reply-pip-live.json`；本次通过不解决此前间歇出现的群聊工具标记正文问题 |
| 原生接口复用与失效对照 | 删除两个私有转发函数，直接使用事件身份和回复接口；私聊、定向群聊及 `@all` 的三份回复载荷与精简前一致。移除群聊 `at=False` 会增加提及，反事实检查失败且正常释放资源 | `qq-native-reply-comparison.json`、`qq-sdk-reply-missing-at-contract.json`；沿用此前配置、过滤及连接异常／取消证据，没有改动协议参数或清理逻辑 |
| wheel／sdist | `packages-45937e81/inspection.json` 核对 36 个应用文件与 schema，排除私人配置；日常和独立环境均已安装 wheel，后者依赖检查通过 | wheel SHA-256 以 `b509b62e` 开头；sdist 包含后续文档提交，应用身份仍为 `45937e81`；同版本 1.0.1 本地候选，未公开发布 |
| 现有 onedir／onefile | `frozen/45937e81-unchanged-cli.json` 重核两个制品指纹及 33 个可达源码文件；本次 QQ 简化仍不在 CLI/TUI EXE 内 | 制品及真实 API 运行仍归属 `6d9b8f86`，本次未重建、未重跑；不能把源码／pip 的 QQ 修复称为 EXE 机器人验收 |
...
| 逐函数必要性／正交性 | 原 `fab35348` 的 1,086 个 callable 全量枚举不等于证明；当前 992 个函数＋79 个 lambda。`necessity-qq-native-reply-delta.json` 在原 SDK 清单基础上再删除 2 个转发包装，保留原生身份、回复载荷及清理行为。同步清理包装的所有分支必要性、真实账号生命周期、2 个旧记忆接口契约仍未完成，不能以运行观察替代全项目删除实验 |
