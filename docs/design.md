...
| 结构／辅助检查 | 8 模块／每模块≤5 文件／最大 498 有效行；总有效行 10,343，较基线少 30。源码 92 项通过；测试自身 5 文件、最大 488 有效行；Ruff 和 whitespace 检查通过 | 逐函数必要性／正交性以及完整运行策略来源尚未全部证明 |
...
新增负向验收缺口：用户要求“仅当前项目”即时记忆时，Coordinator 仍调用了仅支持全局 L2 的 `remember`，感知保存后 Coordinator 才发现范围错误并删除。查库确认该测试记录为 deleted v2，项目与全局索引均无残留；但先误保存仍判为失败。现行设计的 L1 是自动感知，不能把此结果表述为手动项目记忆成功。是否扩展显式作用域已向用户澄清；答复前保持现有 L1／L2 定义，未擅自修改提示词或扩大工具能力。
...
最新 `packages-usage` 已包含分类修复：wheel／sdist 内 36 个应用文件与当前源码逐字节一致，字段契约存在，私人配置与会话未入包；日常 pip 和既有干净环境覆盖安装成功，干净安装 92 项回归通过（33.38 秒），依赖检查通过。日常 pip 恢复 `8e23a68b2a7246ad84b1c9b1c00f2502`，检索到已提交偏好，再由 Worker 实际计算 23+19=42；2 回合／11 次响应，主、普通子 Agent、感知与标题分类及累计值保持正确，正常退出。

最新 `frozen/onedir-usage` 与 `frozen/onefile-usage` 均构建完成并实际调用模型：onedir 恢复原会话后委派 Worker 打开 Example Domain、读取标题并关闭浏览器，累计 4 回合／21 次响应；onefile 在明确的浅目录控制组恢复原会话、读回未变的 `42`，Worker 打开／关闭同一网页，累计 3 回合／28 次响应。二者无缺失 usage，活动 Agent 归零，正常退出。制品哈希及账本汇总分别保留于 `frozen/usage-manifest.json`、`usage-entry-summary.json`；这些是本地 1.0.1 候选，不是公开新版本或真实升级。深目录 onefile、剩余策略来源审计和逐函数证明仍未完成。
