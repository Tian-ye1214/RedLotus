...
**当前不能 LGTM。** 应用候选为 `6e1911b1`，相对重做基线 `ca98b811`：8 个模块、36 个应用 Python 文件，每模块最多 5 文件、每文件最多 498 有效行；总计 10,314 有效行，少 59 行，应用 Python 毛新增 5,952、毛删除 6,158。第三方 Skills 资源另计，不能借此排除自有实现。逐函数必要性与完整发布门槛没有因数量达标而通过。

源码完整辅助回归为 **157 项通过（42.74 秒）**，已安装 wheel 的独立环境同样 **157 项通过（39.19 秒）**，该环境依赖检查通过。覆盖文件外部编辑／删除、LF 与 CRLF 字节恢复、写盘失败、记忆事务、任务取消／恢复、上下文账本、输入顺序、配置、QQ 公网地址／TLS／回退、原生类型化附件、终端中断及资源回收。故障注入与网络夹具使用隔离数据，不能替代真实 API、机器人账号和跨平台验收。Ruff 与 whitespace 检查通过，14 个受保护配置及提示词指纹未变。日常共享 Python 环境的 `pip check` 仍返回六项第三方依赖冲突，见 `qq-tls-daily-pip-check.log`；本次 `--no-deps` 只替换 RedLotus，没有更改冲突软件的依赖。

### 当前候选与真实入口

| 入口／制品 | `6e1911b1` 实际证据 | 边界 |
| --- | --- | --- |
| 源码 QQ 原生 SDK 至模型 | ncatbot 4.4.1.post1 的原生注册、事件分派、`run_frontend`／`start` 均实际执行；私聊 read_file、原生 File 文件 ID 经真实 HTTP 下载 537 字节并回答首行、定向群聊计算共 3 回合／4 次响应，usage 无缺失，退出后 HTTP 池为 0 | `qq-sdk-delivery-source-live.json`；NapCat 启动、文件 URL 返回和收发传输是本地夹具，没有登录账号 |
| 日常 pip 的同一 SDK 场景 | 安装新 wheel，确认模块来自日常环境 site-packages；文件工具和附件回答通过；群聊计算失败，模型返回 DSML 标记正文而非答案。账本封存 3 回合／4 次响应，usage 无缺失，失败退出也释放全部会话和 HTTP 池 | `qq-sdk-delivery-pip-live.json`、`qq-sdk-delivery-pip-analysis.json`；3 个封存回合不等于 3 项成功，没有改题或重复请求刷通过 |
| 配置与失效对照 | 原生配置的 debug／remote_mode／enable_webui_interaction 不再被调用参数覆盖；关闭 WebUI 时不校验无关令牌，启用时仍校验。移除群聊过滤、回复或连接清理后，对应行为确实失败；连接异常和取消仍释放资源 | `qq-sdk-policy-green-contract.json`、`qq-sdk-inactive-webui-green.json`、`qq-sdk-frontend-mutations.json`；定向反事实检查不替代真实 API 验收 |
| wheel／sdist | `packages-6e1911b1/inspection.json` 核对 36 个应用文件与 schema，排除私人配置；日常和独立环境均已安装 wheel，后者依赖检查通过 | wheel SHA-256 以 `8161b5cf` 开头；sdist 包含后续文档提交，应用身份仍为 `6e1911b1`；同版本 1.0.1 本地候选，未公开发布 |
| 现有 onedir／onefile | `frozen/6e1911b1-unchanged-cli.json` 重核两个制品指纹及 33 个可达源码文件；本次修改的 QQ 适配器与附件辅助文件均不在 CLI/TUI EXE 内 | 制品及真实 API 运行仍归属 `6d9b8f86`，本次未重建、未重跑；不能把源码／pip 的 QQ 修复称为 EXE 机器人验收 |
...
安装包失败群聊没有本人工具权限，持久化响应确为文本且无 `tool-call`；没有执行文本中的命令。原始 HTTP 响应未单独抓取，不能进一步归因到服务内部哪一层。首次事后分析错用仅返回计数的账本接口，改从 `SessionFile.model_messages` 读取后完成核对，没有新增 API 调用。该回答质量问题仍未通过，不能因正常清理而关闭。原生 SDK 的两个 WARNING 分别是隔离插件目录首次创建、夹具中 localhost 监听值与 127.0.0.1 主机名不同；均按 SDK 校验源码解释，不算真实连接通过。SDK 的继承环境 `LOG_FORMAT=json` 冲突及密码检查正则差异另有原始记录，测试只调整自身进程和公开夹具，没有修改产品环境或供应方代码。
...
| 门槛 | 当前状态与下一步 |
| --- | --- |
| 深目录 onefile | 原复现场景已关闭：同一深目录实际 API、包内浏览器及退出清理通过。启动目录再加深或制品路径增长仍可能超过 Windows 上限；没有承诺任意路径，也没有修改注册表 |
| 逐函数必要性／正交性 | 原 `fab35348` 的 1,086 个 callable 全量枚举不等于证明；当前 994 个函数＋79 个 lambda。`necessity-qq-sdk-delta.json` 逐项衔接原 15 项清单：删除 3 个旧包装，增加 1 个连接清理边界；原生 QQ 分派及定向失效对照补足部分证据。同步清理包装的所有分支必要性、真实账号生命周期、2 个旧记忆接口契约仍未完成，不能以运行观察替代全项目删除实验 |
...
| 无工具群聊回答 | 当前安装包的真实计算请求得到 DSML 工具标记正文，没有答案；保存的 SDK 响应没有可执行工具调用，权限未放开。保留失败，后续定位服务响应与无工具上下文约束；不把正文解析为工具调用或修改题面取得通过 |
